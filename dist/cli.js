...
program
    ...
    try {
        ...
        const restorePath = await validator.restoreFromBackup(absolutePath, options.timestamp);
...
program
    ...
    try {
        ...
        if (!integrations[options.framework]) {
            console.error(`❌ Unknown framework: ${options.framework}`);
            process.exit(1);
        }
        const integrationCode = integrations[options.framework];
        if (options.output) {
